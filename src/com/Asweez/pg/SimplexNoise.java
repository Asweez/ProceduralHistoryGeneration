...
public class SimplexNoise {
	...
	double frequency, persistence, lacunarity, gain;

	int seed;
	
	public SimplexNoise(int octaveNum, double persistence, double frequency, double lacunarity, int seed){
		this.persistence = persistence;
		this.frequency = frequency;
		this.seed = seed;
		this.lacunarity = lacunarity;
		octaves = new SimplexNoise_octave[octaveNum];
	...
	public double getNoise(int x, int y) {
		...
		for (int i = 0; i < octaves.length; i++) {
			...
			result += octaves[i].noise((double)x * freq, (double)y * freq) * amp;
			maxAmp += amp;
			amp *= persistence;
			freq *= lacunarity;
		}
...
//		result/=maxAmp;
	...
	public double getNoise(int x, int y, int z) {
		...
		for (int i = 0; i < octaves.length; i++) {
			...
			result = result + octaves[i].noise((double)x / frequency, y / frequency, z / frequency) * amplitude;
		}
	...
	public static class SimplexNoise_octave2{
		...
		public static double[][] generateSmoothNoise(int width, int height, int octaves, int seed){
			return generateSmoothNoise(generateWhiteNoise(width, height, seed), octaves);
		}
		...
		public static double[][] generatePerlinNoise(int width, int height, int octaves, double frequency, double lacunarity, double persistence, int seed){
			return generatePerlinNoise(generateWhiteNoise(width, height, seed), octaves, frequency, lacunarity, persistence);
		}
		
		public static double[][] generatePerlinNoise (double[][] baseNoise, int octaveCount, double frequency, double lacunarity, double persistence) {
			int width = baseNoise.length;
			int height = baseNoise[0].length;
			...
			double[][] perlinNoise = new double[width][height]; // an array of doubles initialised to 0
...
//					perlinNoise[i][j] = Math.pow(perlinNoise[i][j], 2);
				}
			}
	...
	public static class SimplexNoise_octave { // Simplex noise in 2D, 3D and 4D
		...
		private static Grad grad4[] = { new Grad(0, 1, 1, 1), new Grad(0, 1, 1, -1), new Grad(0, 1, -1, 1), new Grad(0, 1, -1, -1), new Grad(0, -1, 1, 1), new Grad(0, -1, 1, -1), new Grad(0, -1, -1, 1), new Grad(0, -1, -1, -1), new Grad(1, 0, 1, 1), new Grad(1, 0, 1, -1), new Grad(1, 0, -1, 1), new Grad(1, 0, -1, -1), new Grad(-1, 0, 1, 1), new Grad(-1, 0, 1, -1), new Grad(-1, 0, -1, 1), new Grad(-1, 0, -1, -1), new Grad(1, 1, 0, 1), new Grad(1, 1, 0, -1), new Grad(1, -1, 0, 1), new Grad(1, -1,
				0, -1), new Grad(-1, 1, 0, 1), new Grad(-1, 1, 0, -1), new Grad(-1, -1, 0, 1), new Grad(-1, -1, 0, -1), new Grad(1, 1, 1, 0), new Grad(1, 1, -1, 0), new Grad(1, -1, 1, 0), new Grad(1, -1, -1, 0), new Grad(-1, 1, 1, 0), new Grad(-1, 1, -1, 0), new Grad(-1, -1, 1, 0), new Grad(-1, -1, -1, 0) };
		...
		private static short p_supply[] = { 151, 160, 137, 91, 90, 15, // this
				...
				135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241, 81, 51, 145, 235, 249, 14, 239, 107, 49, 192,
				214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180 };
		...
		public SimplexNoise_octave(int seed) {
			...
			// the seed determines the swaps that occur between the default
			// order
